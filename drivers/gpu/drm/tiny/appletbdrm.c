...
{
	struct appletbdrm_simple_request *request __free(kfree) =
	...
	return appletbdrm_send_request(adev, &request->header, sizeof(*request));
}
...
{
	struct appletbdrm_information *info __free(kfree) = kzalloc(sizeof(*info), GFP_KERNEL);
	struct drm_device *drm = &adev->drm;
	u8 bits_per_pixel;
	u32 pixel_format;
	int ret;
				   ...
				   struct drm_plane_state *state)
{
	...
	struct appletbdrm_fb_request_response *response __free(kfree) = NULL;
	struct appletbdrm_fb_request *request __free(kfree) = NULL;
	struct appletbdrm_fb_request_footer *footer;
